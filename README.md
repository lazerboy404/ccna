...
- **Interfaz gráfica de equipos finales**: al abrir una PC/servidor/laptop aparece una ventana estilo **Windows** y al abrir una **cámara IP** un **panel web** (`http://…/admin`) para configurar IP/máscara/puerta de enlace, más un **Símbolo del sistema funcional** (escribes `ipconfig`, `ip …`, `ping …` y responde). La **CLI** sigue disponible con un clic, y la consola se puede **ampliar (⛶)**; al seleccionar un equipo del diagrama, la consola se lleva a la vista.
- **Consola de PC estilo Windows**: `hostname`, `ipconfig` (`/all`, `/release`, `/renew`, `/flushdns`), `getmac`, `arp -a`, `netstat` (`-e`, `-r`), `route print`, `tracert`, `nslookup`, `systeminfo`, `ver`, `whoami`, `date`, `time`, además de `ping` e `ip` para configurar. El header muestra el **hash del commit** desplegado.
...
- **UI estilo Spatial (glassmorphism)**: paneles traslúcidos con `backdrop-filter: blur(20px) saturate(180%)`, profundidad en el eje Z (`perspective`, `translateZ`), sombras multicapa con luz interna, micro-interacciones 3D en botones y **tilt con el mouse** (módulo ligero `src/lib/tilt.js`, respeta `prefers-reduced-motion`).
...
│   └── tilt.js              # Efecto tilt 3D (Spatial UI)
├── App.jsx
├── main.jsx
